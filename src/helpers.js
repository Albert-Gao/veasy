 ...
 * @returns {object}
 */
export function validatorRunner(fieldInfo) {
  const { value, schema } = fieldInfo;
...
export function createNewState(oldComponentState, fieldState) {
  ...
  return {
    formStatus: {
      isFormOK: oldComponentState.formStatus.isFormOK,
      fields: {
        ...oldComponentState.formStatus.fields,
